...
def test_returns_empty_dict_when_no_bucket_has_enough_samples(tmp_path):
    ...
    _write_marker(videos_dir, "short-1.done", "V1", "Rainy Night Anime Lofi — Amber Hours")
...
def test_computes_a_relative_weight_once_a_bucket_has_enough_samples(tmp_path):
    ...
    videos_dir = tmp_path / "_videos"
    videos_dir.mkdir()
    # "Rainy Night Lofi" bucket: 3 videos averaging 200 views.
    for i, views in enumerate([100, 200, 300], start=1):
        _write_metric(metrics_path, f"RAIN{i}", views)
        _write_marker(videos_dir, f"short-rain-{i}.done", f"RAIN{i}", "Rainy Night Anime Lofi — Amber Hours")
    # "Cozy Cat Lofi" bucket: 3 videos averaging 50 views (worse than channel avg).
    for i, views in enumerate([40, 50, 60], start=1):
        _write_metric(metrics_path, f"CAT{i}", views)
        _write_marker(videos_dir, f"short-cat-{i}.done", f"CAT{i}", "Sleepy Cat Anime Lofi — Amber Hours")

    weights = broll_performance.mood_performance_weights(
        metrics_path=metrics_path, videos_dir=videos_dir, min_samples=3
    )

    assert weights["Rainy Night Lofi"] > 1.0
    assert weights["Cozy Cat Lofi"] < 1.0
...
def test_weights_are_clamped_to_the_configured_bounds(tmp_path):
    ...
    assert weights["Rainy Night Lofi"] == broll_performance.MAX_WEIGHT
    assert weights["Cozy Cat Lofi"] == broll_performance.MIN_WEIGHT
...
def test_ignores_markers_whose_video_id_has_no_metrics_row(tmp_path):
    ...
    _write_marker(videos_dir, "short-1.done", "NO_METRICS", "Rainy Night Anime Lofi — Amber Hours")

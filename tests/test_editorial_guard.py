...
def test_editorial_guard_blocks_generic_successor_templates():
    ...
    assert "generic_signal_through_body_cue" in editorial_issues({"title": "Horses recognize signals through ear position"})
    assert "generic_signal_through_body_cue" not in editorial_issues({"title": "Dolphins recognize signals through call"})
    ...
    assert "generic_movement_changes_title" in editorial_issues({"title": "This movement changes what orangutans do next"})
    assert "generic_movement_changes_title" in editorial_issues({"title": "This first move changes what cows do next"})
    assert "generic_first_movement_reason" in editorial_issues({"title": "Snakes rely on the first movement for a reason"})
    assert "generic_first_move_title" in editorial_issues({"title": "Cows read the moment from one first move"})
    assert "generic_first_move_title" in editorial_issues({"title": "Elephants react differently when the first move appears"})
    assert "generic_first_move_title" in editorial_issues({"title": "Snakes rely on first movement to survive"})
    assert "awkward_ear_movement_changes" in editorial_issues({"title": "Elephants react differently when the ear movement changes"})
    assert "awkward_this_ear_position_changes" in editorial_issues({"title": "This ear position changes what lions do next"})

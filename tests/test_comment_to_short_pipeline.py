...
def test_comment_to_short_recovers_specific_plural_animal_question():
    ...
    assert candidates[0]["title"] == "Ducklings follow the group before they swim"
    assert "group movement" in candidates[0]["script"]
...
def test_comment_to_short_maps_big_cat_question_to_wildlife():
    ...
    assert candidates[0]["title"] == "Lions keep the hunt quiet for a reason"
...
def test_merge_into_queue_skips_duplicate_comment_angle():
    existing = {
        "id": "lion-existing",
        "title": "Lions keep the hunt quiet for a reason",
        "seo_title": "Lions keep the hunt quiet for a reason",
...
def test_merge_into_queue_refreshes_existing_comment_idea():
    ...
    assert merged["stories"][0]["title"] == "Sharks follow one fin cue before the payoff"
...
def test_merge_into_queue_removes_existing_comment_idea_that_no_longer_passes_directly():
    ...
    assert merged["stories"] == []
    assert merged["comment_to_short_removed"] == 1
...
def test_comment_idea_packaging_preserves_viewer_question_hook():
    ...
    assert packaged["title"] == "Lions keep the hunt quiet for a reason"
    assert packaged["hook"] == "Lions keep the hunt quiet for a reason."

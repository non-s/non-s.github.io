...
def test_session_ops_filters_malformed_sequel_opportunities(tmp_path):
    ...
    (videos / "good.done").write_text(json.dumps({
        "video_id": "good",
...
def test_session_ops_filters_malformed_related_targets(tmp_path):
    videos = tmp_path / "_videos"
    videos.mkdir()
    (videos / "source.done").write_text(json.dumps({
        "video_id": "source",
...
def test_session_ops_skips_malformed_related_sources(tmp_path):
    videos = tmp_path / "_videos"
    videos.mkdir()
    (videos / "bad-source.done").write_text(json.dumps({
        "video_id": "bad-source",
        "title": "Lions use their ears to use",
        "category": "wildlife",
        "series": "Wildlife",
    }), encoding="utf-8")
    (videos / "good-target.done").write_text(json.dumps({
...
def test_session_ops_uses_recent_clean_sources_when_newest_markers_are_malformed(tmp_path):
    videos = tmp_path / "_videos"
    videos.mkdir()
    (videos / "01-good-source.done").write_text(json.dumps({
    ...
    (videos / "02-good-target.done").write_text(json.dumps({
        "video_id": "good-target",
        "title": "Dolphins recognize signals through call",
        "category": "ocean",
        "series": "Ocean",
    ...
    for index in range(5):
        (videos / f"9{index}-bad.done").write_text(json.dumps({
...
def test_session_ops_orders_latest_markers_by_uploaded_at(tmp_path):
    videos = tmp_path / "_videos"
    videos.mkdir()
    (videos / "99-old.done").write_text(json.dumps({
        "video_id": "old",
        "title": "Octopus skin warning",
        "category": "ocean",
        "series": "Ocean",
    ...
    for index in range(5):
        (videos / f"0{index}-new.done").write_text(json.dumps({

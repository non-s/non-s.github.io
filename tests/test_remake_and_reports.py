...
def test_remake_engine_builds_backlog(tmp_path: Path):
    _write(
        tmp_path / "_data" / "analytics" / "latest.json",
        {
            "remake_candidates": [
                {
                    "video_id": "abc",
...
def test_weekly_report_contains_core_sections(tmp_path: Path):
    ...
    _write(
        tmp_path / "_data" / "ops_guardian.json",
        {
    ...
    _write(
        tmp_path / "_data" / "remake_backlog.json",
        {
            "remakes": [{"source_title": "Cows remember faces", "action": "make sequel"}],
        },
    )
    _write(
        tmp_path / "_data" / "trend_radar.json",
        {
            "topics": [
                {
                    "animal": "orca",
                    ...
                    "trend_score": 88,
                    "mentions": 3,
                    "top_titles": ["Rare orca behavior"],
                }
            ],
        },
    )
    _write(
        tmp_path / "_data" / "early_performance.json",
        {
            "top_velocity": [
                {
                    "video_id": "bad1",

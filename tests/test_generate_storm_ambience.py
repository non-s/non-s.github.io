...
def test_build_metadata_uses_ai_copy_when_available(tmp_path, monkeypatch):
    ...
    meta = storm._build_metadata("deep sleep", 3600.0, video_path, slug="s-ai", broll_meta={})
...
def test_build_metadata_always_appends_the_synthesized_disclosure(tmp_path, monkeypatch):
    ...
    meta = storm._build_metadata("focus", 3600.0, video_path, slug="s-disc", broll_meta={})
...
def test_build_metadata_uses_hours_label_for_long_videos(tmp_path):
    ...
    meta = storm._build_metadata("deep sleep", 3.5 * 3600, video_path, slug="ambience-1700000000-1234", broll_meta={})
...
def test_build_metadata_uses_minutes_label_under_an_hour(tmp_path):
    ...
    meta = storm._build_metadata("focus", 45 * 60, video_path, slug="ambience-1700000000-5678", broll_meta={})
...
def test_build_metadata_never_rounds_a_short_duration_down_to_zero_minutes(tmp_path):
    ...
    meta = storm._build_metadata("focus", 30.0, video_path, slug="s-0", broll_meta={})
...
def test_build_metadata_tags_lead_with_the_scene_then_default_tags(tmp_path):
    ...
    meta = storm._build_metadata("deep sleep", 3600.0, video_path, slug="s-3", broll_meta={})
...
def test_build_metadata_publish_slot_uses_the_storm_prefix(tmp_path):
    ...
    meta = storm._build_metadata("focus", 3600.0, video_path, slug="s-4", broll_meta={})
...
def test_build_metadata_carries_real_broll_source_fields(tmp_path):
    ...
    meta = storm._build_metadata("focus", 3600.0, video_path, slug="s-5", broll_meta=broll_meta)
...
def test_compose_storm_builds_pure_rain_ffmpeg_command(tmp_path, monkeypatch):
    calls = []
    ...
    assert storm._compose_storm(segment, rain, out_path, 120.0) is True
...
def test_compose_storm_returns_false_on_ffmpeg_failure(tmp_path, monkeypatch):
    ...
    ok = storm._compose_storm(tmp_path / "s.mp4", tmp_path / "r.wav", tmp_path / "out.mp4", 60.0)
    assert ok is False

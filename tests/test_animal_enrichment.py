...
def test_gbif_species_context_returns_taxonomy(monkeypatch, tmp_path):
    ...
    response.json.return_value = {
        "matchType": "EXACT", "usageKey": 2435099, "scientificName": "Octopus vulgaris",
        "canonicalName": "Octopus vulgaris", "rank": "SPECIES", "family": "Octopodidae",
    }
    with patch.object(animal_enrichment, "_session") as factory:
        session = MagicMock(); session.get.return_value = response; factory.return_value = session
...
def test_commons_image_rejects_noncommercial_license(monkeypatch, tmp_path):
    monkeypatch.setattr(animal_enrichment, "_CACHE_DIR", tmp_path)
    response = MagicMock(status_code=200)
    response.json.return_value = {"query": {"pages": {"1": {"imageinfo": [{

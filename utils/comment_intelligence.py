...
_STOPWORDS = {
    "about", "after", "again", "animal", "animals", "because", "brief",
    "could", "great", "learn", "maybe", "please", "really", "short",
    "shorts", "thanks", "their", "there", "these", "thing", "video",
    "watch", "where", "which", "while", "would", "youtube",
}

_ANIMALS = {
    "ant", "bat", "bear", "bee", "bird", "butterfly", "cat", "chicken",
    "chimpanzee", "cow", "crocodile", "deer", "dog", "dolphin", "duck",
    "eagle", "elephant", "fish", "fox", "goat", "gorilla", "horse",
    "leopard", "lion", "monkey", "octopus", "owl", "parrot", "penguin",
    "seal", "shark", "snake", "tiger", "turtle", "whale", "wolf",
}
...
def analyze_comments(comments: list[dict], limit: int = 8) -> dict:
    ...
    return {
        "comments_sampled": len([c for c in comments if clean_comment(str(c.get("text") or c.get("textDisplay") or c.get("textOriginal") or ""))]),
        ...
        "content_prompts": [
            f"Answer this viewer question: {item['text']}"
            for item in top_questions[:5]
        ],
    }

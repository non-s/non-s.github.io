...
def decide_cadence(data_root: Path, *, now: datetime | None = None, manual: bool = False) -> CadenceDecision:
    ...
    try:
        daily_limit = min(8, max(1, int(os.environ.get("LIQUID_WIRE_DAILY_PUBLICATION_LIMIT", "4"))))
    except ValueError:
        daily_limit = 4
    try:
    ...
    if recent and current - max(recent) < timedelta(hours=4):
        return CadenceDecision(False, "minimum four-hour learning interval not reached", len(recent), daily_limit)

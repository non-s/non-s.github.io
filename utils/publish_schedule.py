...
def _event_schedule_cron(env: Mapping[str, str] | None = None) -> str:
    env = env or os.environ
    return str(env.get("PUBLISH_EVENT_SCHEDULE_CRON") or env.get("GITHUB_EVENT_SCHEDULE") or "").strip()


def _event_schedule_max_delay(env: Mapping[str, str] | None = None) -> int:
    return max(0, _env_int("PUBLISH_EVENT_MAX_DELAY_MINUTES", 360, env))


def _recovery_delay_minutes(env: Mapping[str, str] | None = None) -> int:
...
def feature_flags(env: Mapping[str, str] | None = None) -> dict:
...
def canonical_slots(env: Mapping[str, str] | None = None) -> list[str]:
...
def _schedule_slots(schedule: dict | None = None, env: Mapping[str, str] | None = None) -> list[str]:
...
def active_slot_label(
    now: datetime | None = None, schedule: dict | None = None, env: Mapping[str, str] | None = None
) -> str:
...
def next_slot(now: datetime | None = None, schedule: dict | None = None, env: Mapping[str, str] | None = None) -> str:

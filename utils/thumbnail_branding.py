...
def _draw_wordmark(im: Image.Image, xy: tuple[int, int], size: int) -> None:
    d = ImageDraw.Draw(im, "RGBA")
    font = _font(size)
    d.text(xy, "AMBER HOURS", font=font, fill=CREAM, stroke_width=max(2, size // 14), stroke_fill=PURPLE_TOP)


def brand_short_thumbnail(thumb_path: Path, mood: str) -> None:
    """In-place: overlay a bottom scrim + skyline/moon accent + hook text
    onto the real full-bleed vertical still (1080x1920) _extract_thumbnail()
    already grabbed. The footage stays the dominant visual; only a strip at
    top/bottom carries the brand."""
    ...
    corner_glow = _amber_glow(w, h, int(w * 0.86), int(h * 0.1), int(w * 0.35))
    overlay = Image.alpha_composite(overlay, corner_glow)
    overlay = Image.alpha_composite(overlay, _moon(w, h, int(w * 0.86), int(h * 0.08), int(w * 0.05)))
...
def brand_mix_thumbnail(thumb_path: Path, mood: str) -> None:
    ...
    canvas = Image.alpha_composite(canvas, _amber_glow(w, h, int(w * 0.86), int(h * 0.42), int(w * 0.24)))
    canvas = Image.alpha_composite(canvas, _moon(w, h, int(w * 0.8), int(h * 0.16), int(w * 0.035)))

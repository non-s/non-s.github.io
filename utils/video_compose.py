...
def extract_thumbnail_from_video(video_path: Path,
                                  output_path: Path,
                                  timestamp_s: float = 15.0) -> bool:
    """Grab a single frame from the composed Short and save as JPEG.

    Used as the per-Short thumbnail so the YouTube preview shows the
    actual animal footage rather than a generated title card. The
    operator is expected to override the auto-thumb manually via
    YouTube Studio for the videos they want bespoke art on; this just
    gives them a "good enough" placeholder so the channel never ships
    a Short with a generic title-card preview.

    `timestamp_s` defaults to 15 s — the middle of a 30 s Short, well
    past the intro card slice and squarely in the b-roll body where
    the most visually rich frame lives.

    Returns True iff the JPEG was written and clears the 5 KB sanity
    floor downstream checks impose.
    """
    ...
    try:
        ...
        log.warning("ffmpeg thumbnail extract timed out")
        return False
    if result.returncode != 0:
        log.warning("ffmpeg thumbnail extract failed: %s", result.stderr[-400:])
        return False
    ok = output_path.exists() and output_path.stat().st_size > 5 * 1024
    if ok:
        log.info("  🖼  Thumbnail extracted from %.1fs of %s",
                 timestamp_s, video_path.name)
    return ok

...
def build_plan(*, latest: dict, health: dict, ops: dict, trend: dict) -> dict:
    ...
    hot_categories = [item for item in list(recs.get("hot_categories") or []) if str(item).lower() not in paused_set]
    if not hot_categories:
        hot_categories = [item for item in ("farm", "wildlife", "birds", "ocean", "dogs") if item not in paused_set]
        ...
        days.append(
            {
                ...
                "avoid": paused[:3],
                "goal": (
                    "raise retention above 62%"
                    if float(latest.get("avg_view_pct", 0) or 0) < 62
                    else "scale strongest series"
                ),
            }
        )
    return {

...
def _build_prompt(story: dict, target_lang: str) -> str:
    ...
    return (
        ...
        f"prÃ³prios ossos.').\n"
        f"  - `script`: 42-65 words. First sentence MUST match the "
        f"translated hook exactly. Then 1-2 surprising facts about "
        f"the subject; keep one small host reaction if present; close "
        f"with a one-line question for the comments.\n"
